#!/usr/bin/env python3
"""
3D Pose Localizer Node using MediaPipe for ROS 2 Humble.
...
class PoseLocalizer3DNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('publish_all_landmarks', False)
        self.declare_parameter('input_image_topic', '/camera/image_raw')
        self.declare_parameter('output_poses_topic', '/human_poses_3d')
        ...
        self.publish_all_landmarks = self.get_parameter('publish_all_landmarks').value
        input_topic = self.get_parameter('input_image_topic').value
        output_topic = self.get_parameter('output_poses_topic').value
        ...
        # Publishers
        self.pose_3d_pub = self.create_publisher(PoseArray, output_topic, 10)
        
        if self.publish_all_landmarks:
            self.landmarks_pub = self.create_publisher(PoseArray, output_topic + '_landmarks', 10)
    ...
    def get_body_orientation_3d(self, landmarks):
        """
        Get body orientation (yaw) in degrees from landmarks.
        """
        try:
            left_shoulder = landmarks[self.LEFT_SHOULDER]
            right_shoulder = landmarks[self.RIGHT_SHOULDER]
            ...
            yaw = np.degrees(np.arctan2(body_vec[1], body_vec[0]))
            return yaw
        except:
            return 0.0
